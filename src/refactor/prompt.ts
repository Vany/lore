...
export function combinePrompt(i: CombineInput): string {
  ...
  return [
    `Two independent models were each asked what is worth refactoring in ${where}, without seeing each`,
    "other's answer. You did not read the code yourself and are not being asked to — your job is to merge",
    "their suggestions into ONE list a person can actually act on.",
    "",
    ...
    "named by only one model: a real idea one model had and the other missed is still a real idea.",
    "",
