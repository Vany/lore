...
| **a paid route is now answering reviews** | a ticket, and the notice the 2026-08-16 incident did not have: at 05:06 a subscription hit its billing-cycle limit, the chain walked onto a metered twin, and the first thing to speak was a daily ceiling four hours and $101.36 later. Fires on the first call each UTC day that runs on a route LORE reached rather than one the operator configured, naming the tier, the route and that call's cost. An EVENT and not a threshold — no total is consulted, so D-121 stands — and it cannot fire under `LORE_ALLOW_METERED=0`, because no metered call happens |
